...
import { Heart, MapPin as PinIcon, MessageCircle, Share2, Loader2, Eye, MapPin } from 'lucide-react';
...
export default function StoryCard({ story, userLocation, teleportedLocation, onClick }: StoryCardProps) {
  ...
  return (
    ...
    >
      ...
      <CardHeader className="pb-2">
        ...
        <h3 className="font-semibold text-sm leading-snug line-clamp-2">{story.title}</h3>
      </CardHeader>

      <CardContent className="pb-2">
        <p className="text-muted-foreground text-xs leading-relaxed">{preview}</p>
        ...
        <div className="flex items-center gap-1 w-full">
          ...
          >
            {isPinLoading ? (
              ...
              <PinIcon className={`w-3.5 h-3.5 ${hasPinned ? 'fill-current' : ''}`} />
            )}
