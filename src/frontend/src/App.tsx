...
export default function App() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: intentional — only reruns on auth change
  useEffect(() => {
    const checkInteractivity = () => {
      ...
      if (body && window.getComputedStyle(body).pointerEvents === "none") {
        console.warn(
          "Detected pointer-events: none on body, restoring interactivity",
        );
      ...
      if (root && window.getComputedStyle(root).pointerEvents === "none") {
        console.warn(
          "Detected pointer-events: none on root, restoring interactivity",
        );
